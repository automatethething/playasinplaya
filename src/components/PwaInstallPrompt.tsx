...
export function PwaInstallPrompt({
  ...
  const [showIosHelp] = useState(() => typeof window !== "undefined" && isIosSafari());
  const [visible, setVisible] = useState(() => {
    if (typeof window === "undefined" || !isIosSafari()) return false;
  ...
  return (
    <section style={{ border: "1px solid #ddd", borderRadius: 16, padding: 16, background: "#fff", marginBottom: 16, position: "relative" }}>
      <button type="button" onClick={dismiss} aria-label="Dismiss install prompt" style={{ position: "absolute", top: 10, right: 10, minWidth: 44, minHeight: 44, border: 0, background: "transparent", cursor: "pointer", fontSize: 18 }}>×</button>
      ...
      {showIosHelp && !deferredPrompt ? (
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button type="button" onClick={install} style={{ minHeight: 44, padding: "10px 14px", borderRadius: 10, border: 0, background: "#111", color: "#fff", cursor: "pointer" }}>Add app</button>
          <button type="button" onClick={dismiss} style={{ minHeight: 44, padding: "10px 14px", borderRadius: 10, border: "1px solid #ddd", background: "#fff", cursor: "pointer" }}>Not now</button>
        </div>
      )}
    </section>
  );
